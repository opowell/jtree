...
class StaticServer {
    ...
    constructor(jt) {
        ...
        //////////////////////////////
        // FILES TO SERVE
        expApp.use('', express.static(path.join(global.jt.path, jt.settings.participantUI)));
        expApp.use('/help', express.static(path.join(global.jt.path, jt.settings.helpPath)));
        expApp.use('/source', express.static(path.join(global.jt.path, jt.settings.adminUIsSharedPath)));
        ...
        expApp.use('/participant', express.static(path.join(global.jt.path, jt.settings.participantUI)));
        expApp.use('/shared', express.static(path.join(global.jt.path, jt.settings.sharedUI)));
        ...
        this.server.on('listening', () => {
            ...
            console.log('jtree ' + jt.version + ': ' + protocol + self.ip + ':' + self.port + '/admin');
    ...
    defaultAdminUIPath() {
        return path.join(this.adminUIsPath(), global.jt.settings.defaultAdminUI);
    }

    adminUIsPath() {
        return path.join(global.jt.path, global.jt.settings.adminUIsPath);
    }
     ...
     */
    adminUIsSharedPath() {
        return path.join(this.adminUIsPath(), global.jt.settings.adminUIsSharedPath);
    }
    ...
    generateSharedJS(inFile, outFile) {
        var fn = path.join(global.jt.path, inFile) // file with marker
        var newFN = path.join(global.jt.path, outFile) // actual file to be sent to clients and admins
        try {
    ...
    sendParticipantPage(req, res, pId, sessionId) {
        ...
        if (sessionId == null || sessionId == undefined) {
            ...
            session = Utils.findById(global.jt.data.sessions, sessionId);
        }
        ...
        if (sessionId != null && session === null) {
            res.sendFile(path.resolve(global.jt.path, './' + global.jt.settings.participantUI + '/invalidSession.html'));
        } else {
            if (session != null) {
                session.sendParticipantPage(req, res, pId);
            } else {
                res.sendFile(path.join(global.jt.path, global.jt.settings.participantUI + '/readyClient.html'));
            }
        }
    }
