...
server.startSessionFromQueue = function(id) { jt.socket.emit('startSessionFromQueue', {qId: id, userId: window.Cookies.get('userId')}); }
...
server.createSessionAndAddApp = function(appId, options) {
    jt.socket.emit('createSessionAndAddApp', {appId: appId, options: options, userId: window.Cookies.get('userId')});
}

server.sessionCreate = function() {
    jt.socket.emit('sessionCreate', window.Cookies.get('userId'));
}
